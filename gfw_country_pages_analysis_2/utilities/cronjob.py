...
parser = argparse.ArgumentParser(
    description="Pass environment to kick off gfw-sync cron job."
)
parser.add_argument(
    "--environment",
    "-e",
        ...
        # Otherwise assume it's a list of dates
            ...
            # assume instead that we have a list of day-of-the-week names
                ...
                if not set(day_list).issubset(valid_day_list):
                    raise ValueError(
                        "Day list {} not subset of all valid day names".format(day_list)
                    )
...
def main():
    ...
    for layername, layerdef in all_layer_dict.iteritems():
        ...
        if update_layer_today:
            ...
            subprocess.call(
                [python_exe, "gfw-sync.py", "-l", layername, "-e", args.environment]
            )
