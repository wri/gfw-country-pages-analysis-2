...
def climate_table_update(df):
    ...
    log.info(
        "Grouping by week and year, summing alerts, above_ground_carbon_loss and loss_ha"
    )
...
def cumsum(df):
    ...
    merged["cumulative_deforestation"] = (
        merged.groupby(join_fields)["loss_ha"].cumsum().round(4)
    )
    merged["cumulative_emissions"] = (
        merged.groupby(join_fields)["above_ground_carbon_loss"].cumsum().round(4)
    )
